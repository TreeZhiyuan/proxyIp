...
def website_proxy():
    ...
    print('| => 2.快代理: ' + colored('https://www.kuaidaili.com', 'yellow') + '  |')
    print('+=========================================+')
...
def mainFunction():
    ...
    optParser.add_option('-r', '--reconnect', metavar='MAXNUM', type='int', dest='reconnect', action='store',
                         help='test the proxy ip reconnect maximumn')
    optParser.add_option('-c', '--check', dest='check', action='store_true',
                         help='check the reporter proxies was available')
    optParser.add_option('-f', '--filename', type='string', metavar='FILE', dest='filename', action='store',
                         help='specify a host proxy file')
    optParser.add_option('-o', '--output', metavar='OFILE', dest='output', action='store',
                         help='output the file to disk,defalut:[stdout]')
